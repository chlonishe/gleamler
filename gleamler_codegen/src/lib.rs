            ...
            match ident_str.as_str() {
                ...
                "NifResult" => {
                    if let Some(inner) = generic_args.first() {
                        ...
                        Ok("Result(Nil, dynamic.Dynamic)".into())
                    }
                }
...
pub fn generate_gleam(funcs: &[NifFunc], types: &[GleamCustomType], erl_module: &str) -> String {
    ...
    let mut find_resource_params = |t: &str| {
        let mut cur = t;
        while let Some(start) = cur.find("Resource(") {
            let after = &cur[start + 9..];
            if let Some(end) = after.find(')') {
                let inner = &after[..end];
                if inner != "Nil" && !inner.is_empty() {
                    resource_types.insert(inner.to_string());
                }
                cur = &after[end + 1..];
            } else {
                break;
    ...
    for f in funcs {
        find_resource_params(&f.ret);
        for (_, t) in &f.args {
            find_resource_params(t);
        }
    }

    let has_yielder = resource_types.contains("Yielder");
    if has_yielder {
        has_option = true;
    }
    ...
    if has_dict {
        imports.push("import gleam/dict");
    }
    if has_dynamic {
    ...
    if has_yielder {
        imports.push("import gleam/yielder");
    }
    ...
    if has_resource {
        ...
        for res_ty in &resource_types {
            if !types.iter().any(|ct| ct.name == *res_ty) {
                out.push_str(&format!("\npub type {} {{\n  {}\n}}\n", res_ty, res_ty));
            }
        }
    }
...
"#;
    ...
    fn type_to_gleam_resource_arc() {
        ...
        assert_eq!(type_to_gleam(&ty), "Resource(MyStruct)");
    }
