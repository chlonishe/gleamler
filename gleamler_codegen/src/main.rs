...
fn main() {
    ...
    let file = parse_file(&lib_rs).expect("failed to parse lib.rs");
...
fn parse_nif_function(func: ItemFn) -> Option<NifFunc> {
    ...
    for (idx, arg) in func.sig.inputs.iter().enumerate() {
        if let FnArg::Typed(pat_type) = arg {
            ...
            // Skip the leading Env argument so the arity matches the proc-macro.
...
fn generate_erl(funcs: &[NifFunc]) -> String {
...
fn generate_gleam(funcs: &[NifFunc]) -> String {
