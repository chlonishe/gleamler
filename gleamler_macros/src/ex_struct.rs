...
fn gen_decoder(ctx: &Context, fields: &[&Field], atoms_module_name: &Ident) -> TokenStream {
    ...
    super::encode_decode_templates::decoder(
        ...
        quote! {
            ...
            let module: ::gleamler::Atom = term.map_get(atom_struct())?.decode()?;
            if module != atom_module() {
                return Err(::gleamler::Error::RaiseAtom("invalid_struct"));
            }
...
fn gen_encoder(
    ...
    let mut keys = vec![quote! { ::gleamler::Encoder::encode(&atom_struct(), env) }];
    let mut values = vec![quote! { ::gleamler::Encoder::encode(&atom_module(), env) }];
    if add_exception {
        keys.push(quote! { ::gleamler::Encoder::encode(&atom_exception(), env) });
        values.push(quote! { ::gleamler::Encoder::encode(&true, env) });
    }
