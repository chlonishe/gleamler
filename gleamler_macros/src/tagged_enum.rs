    ...
    super::encode_decode_templates::decoder(
        ...
        quote! {
            ...
            if let Ok(unit) = ::gleamler::Atom::from_term(term) {
                #(#unit_decoders)*
            } else if let Ok(tuple) = ::gleamler::types::tuple::get_tuple(term) {
                let name = tuple
                    .get(0)
                    .and_then(|&first| ::gleamler::Atom::from_term(first).ok())
                    .ok_or(::gleamler::Error::RaiseAtom("invalid_variant"))?;
                #(#named_unnamed_decoders)*
            }

            Err(::gleamler::Error::RaiseAtom("invalid_variant"))
        },
    )
}
...
fn gen_unnamed_decoder<'a>(
    ...
    let decoded_field = &fields_iter
        ...
        .map(|(i, f)| {
            ...
            let ty = &f.ty;
            quote! {
                <#ty as ::gleamler::Decoder>::decode(tuple[#i]).map_err(|_| ::gleamler::Error::RaiseTerm(
    ...
    quote! {
        if name == #atom_fn() {
            if tuple.len() - 1 != #len {
                return Err(::gleamler::Error::RaiseTerm(Box::new(format!(
...
fn gen_unit_encoder(enum_name: &Ident, variant_ident: &Ident, atom_fn: Ident) -> TokenStream {
    quote! {
        #enum_name :: #variant_ident => ::gleamler::Encoder::encode(&#atom_fn(), env),
    }
}
