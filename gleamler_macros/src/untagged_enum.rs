...
use syn::{self, Fields, Variant, spanned::Spanned};
...
fn gen_decoder(ctx: &Context, variants: &[&Variant]) -> TokenStream {
    ...
    let variant_defs: Vec<_> = variants
        ...
        .map(|variant| {
            ...
            quote! {
                if let Ok(inner) = <#field_type as ::gleamler::Decoder>::decode(term) {
...
fn gen_encoder(ctx: &Context, variants: &[&Variant]) -> TokenStream {
    ...
    let variant_defs: Vec<_> = variants
        ...
        .map(|variant| {
            ...
            quote! {
                #enum_name :: #variant_name ( ref inner ) => ::gleamler::Encoder::encode(&inner, env),
            }
        })
        .collect();
